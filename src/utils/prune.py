...
class Pruner:
    ...
    def compute_sparsity(self, step):
        if self.args.ramp_type == "linear":
            rate_of_increase = (
                self.args.final_sparsity - self.args.initial_sparsity
            ...
            sin_inner = (
                math.pi
                / 2
                * (step % self.args.ramp_cycle_step)
    ...
    def global_prune(self, model, prune_compute):
        if self.args.prune_type == "weight":
            ...
            masks = []
            for g, reformed_shape, grad in zip(
                unit_weight_norm, reformed_shapes, weights
            ):
    ...
    def local_prune(self, model, prune_compute):
        for module in model.modules():
            if hasattr(module, "mask"):
                mask_sparsity = round(
                    1.0
                    - np.sum(module.mask.detach().cpu().numpy())
                    / module.mask.detach().cpu().numpy().size,
                    2,
                )
                if mask_sparsity <= self.args.final_sparsity:
                    if self.args.carry_mask:
                        ...
                        module_mask = prune(
                            module.weight, prune_compute, self.args.prune_type
                        )
    ...
    def union_mask(self, model, step):
        ...
        if step < (self.end_step - self.args.ramp_cycle_step):
            if (step % self.args.ramp_cycle_step == 0) and (step != 0):
                ...
                for module in model.modules():
                    if hasattr(module, "mask"):
                        ...
                        self.union_masks[count] = 1.0 - (
                            (1.0 - self.union_masks[count]) * (1.0 - module.mask.data)
                        )
                        union_mask_sparsity = round(
                            1.0
                            - np.sum(self.union_masks[count].detach().cpu().numpy())
                            / self.union_masks[count].detach().cpu().numpy().size,
                            2,
                        )
                        # Reset the masks
            ...
            logger.info(
                "Initial sparsity updated to {}".format(self.args.initial_sparsity)
            )
