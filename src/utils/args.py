...
parser.add_argument('--lr_cycle',
                    default='full',
                    type=str,
                    help='Full or half cycle')
parser.add_argument('--up_step',
                    default=5000,
                    type=int,
                    help='Cyclic lr step size')
parser.add_argument('--down_step',
                    default=5000,
                    type=int,
                    help='Cyclic lr step size')
...
parser.add_argument('--eval_step',
                    default=1000,
                    type=int,
                    help='Eval every this steps')
parser.add_argument('--batch_size', default=100, type=int, help='Batch size')
parser.add_argument(
    '--output_dir',
    type=str,
    help='Output directory for storing ckpts. Default is in runs/hparams')
parser.add_argument('--use_colab',
                    type=bool,
                    default=False,
                    help='Use Google colaboratory')

# sparsity params
parser.add_argument('--ramping',
                    type=bool,
                    default=False,
...
parser.add_argument('--carry_mask',
                    type=bool,
                    default=False,
                    help='Carry mask in ramping pruning')
parser.add_argument('--initial_sparsity',
                    default=0.0,
                    type=float,
                    help='Initial sparsity')
parser.add_argument('--final_sparsity',
                    default=0.75,
                    type=float,
                    help='Final sparsity')
parser.add_argument('--start_step',
                    default=0,
                    type=float,
                    help='Pruning start step')
parser.add_argument('--end_step',
                    default=0.2,
                    type=float,
                    help='Stop pruning at this step')
parser.add_argument('--prune_freq',
                    default=100,
                    type=int,
                    help='Prune every x steps')
parser.add_argument('--global_prune',
                    default=False,
                    type=bool,
                    help='Layer wise pruning')
parser.add_argument('--prune_type',
                    default='weight',
                    type=str,
                    help='Weight or unit pruning.')
parser.add_argument('--ramp_type',
                    default='linear',
                    type=str,
                    help='Ramp type: linear, full_cycle, half_cycle')
parser.add_argument('--ramp_cycle_step',
                    default=None,
                    type=int,
                    help='Ramp cycle step')

# mask params
parser.add_argument('--union_mask',
                    default=False,
                    type=bool,
                    help='Take the union of masks')
...
def get_args():
  ...
  if args.model_type == 'dense':
    args.output_dir = '{}/{}/lr_{}/{}'.format(args.dataset, args.model,
                                              args.lr_schedule, args.run_name)
  else:
    args.output_dir = '{}/{}/lr_{}/{}/{}_{}_{}/{}/{}'.format(
        args.dataset, args.model, args.lr_schedule, args.final_sparsity,
        args.start_step, args.end_step, args.prune_freq, args.ramp_type,
        args.run_name)
  if args.use_colab:
