...
def GraSP_fetch_data(dataloader, num_classes, samples_per_class):
  ...
  X, y = torch.cat([torch.cat(_, 0) for _ in datas
...
def GraSP(net,
  ...
  for it in range(num_iters):
    ...
    inputs, targets = GraSP_fetch_data(train_dataloader, num_classes,
                                       samples_per_class)
    N = inputs.shape[0]
  ...
  for idx, layer in enumerate(net.modules()):
    if isinstance(layer, nn.Conv2d) or isinstance(layer, nn.Linear):
      grads[old_modules[
          idx]] = -layer.weight.data * layer.weight.grad  # -theta_q Hg
  ...
  print(
      torch.sum(torch.cat([torch.flatten(x == 1) for x in keep_masks.values()
                          ])))
