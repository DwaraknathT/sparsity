...
def save_model(step, net, optimizer, directory, filename):
    logger.info("Saving..")
    state = {"net": net.state_dict(), "optimizer": optimizer.state_dict(), "step": step}
...
def mask_sparsity(net):
    ...
    for module in net.modules():
        if hasattr(module, "mask"):
            ...
            sparsities.append(round(1.0 - np.sum(mask) / mask.size, 2))
    return sparsities
...
def get_model(args):
    ...
    if args.optim == "sgd":
        optimizer = optim.SGD(
            params, lr=args.lr, momentum=0.9, weight_decay=5e-4, nesterov=True
        )
